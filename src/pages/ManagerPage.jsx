      ...
      {tab==="platforms"&&(
        <div style={{ padding:"16px 20px" }}>
          <div style={{ display:"flex",alignItems:"center",gap:12,marginBottom:16,flexWrap:"wrap" }}>
            ...
            <Select value={statPlatform} onChange={v=>setStatPlatform(v)} style={{ background:T.surface,border:`1px solid ${T.border}`,color:T.sub,padding:"6px 10px",borderRadius:7,fontSize:12 }} options={[{value:"",label:"Все платформы"},...geoPlatformsAll.map(p=>({value:p.id,label:p.name}))]}/>
          </div>
