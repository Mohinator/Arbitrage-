    ...
    <>
      ...
      <div style={{ overflowX:"auto",border:`1px solid ${T.border}`,borderRadius:10 }}>
        <table style={{ width:"100%",borderCollapse:"collapse" }}>
          <thead>
            <tr><th style={{ ...S.th,padding:"6px 8px" }} colSpan={13}>ЛИДЫ{readonly?" (только просмотр)":""}</th></tr>
            <tr>
...
function ManagerPage({ manager, onLogout }) {
  ...
  const [viewingManager, setViewingManager] = useState(null); // manager id to view
  ...
  const load = async () => {
    const [{ data:p },{ data:pl },{ data:rd },{ data:prd },{ data:ug },{ data:g },{ data:am }] = await Promise.all([
    ...
    setGeos(g||[]); setAllManagers(am||[]);
    const myGeoList=(ug||[]).map(u=>u.geos).filter(Boolean);
    setMyGeos(myGeoList);
    if (myGeoList.length>0&&!activeGeo) setActiveGeo(myGeoList[0].id);
  ...
  const genAutomation = () => {
    ...
    Object.entries(byPlat).forEach(([pid,pps])=>{
      ...
      for(let a=0;a<300;a++){
        ...
        let rem=needed;
        for(let r=0;r<9&&rem>0;r++) for(const pp of plans){ if(pp.amts.length>=pp.slots||rem<=0) continue; const amt=genRdAmount(min); pp.amts.push(amt); rem-=amt; }
    ...
    <div style={{ minHeight:"100vh",background:T.bg,color:T.text,fontFamily:"'Inter',sans-serif" }}>
          ...
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12 }}>
            {[["Дата","date","date"],["Продукт","platform_id","select"],["Имя лида","name","text"],["SUB18","sub18","text"],["Депозит (€)","deposit","number"]].map(([l,k,t])=>(
              <div key={k} style={{ gridColumn:k==="name"?"1/-1":undefined }}>
                ...
                {t==="select"?<select value={leadForm[k]} onChange={e=>setLeadForm(f=>({...f,[k]:e.target.value}))} style={IS}><option value="">Выбери платформу</option>{platforms.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select>
      ...
      {tab==="team"&&(
        <div style={{ padding:"16px 20px" }}>
          ...
          {myGeos.map(geo=>{
            const geoManagers2=allManagers.filter(m=>m.id!==manager.id);
            return(
              <div key={geo.id} style={{ marginBottom:28 }}>
                ...
                <div style={{ display:"flex",gap:10,flexWrap:"wrap",marginBottom:16 }}>
                  {geoManagers2.map(m=>{
                    ...
                    const isViewing=viewingManager===m.id;
                    return(
                      <div key={m.id} onClick={()=>setViewingManager(isViewing?null:m.id)} style={{ background:isViewing?`linear-gradient(135deg,${dark?"#1e3a5f":"#dbeafe"},${dark?"#1e2235":"#eff6ff"})`:T.surface,border:`1px solid ${isViewing?"#6366f1":T.border}`,borderRadius:10,padding:"12px 16px",cursor:"pointer",transition:"all .2s",minWidth:160 }}>
                ...
                {viewingManager&&(
                  <div>
                    <div style={{ display:"flex",alignItems:"center",gap:10,marginBottom:12 }}>
                      <span style={{ color:T.text,fontWeight:600,fontSize:14 }}>{allManagers.find(m=>m.id===viewingManager)?.name}</span>
                      <span style={{ color:T.muted,fontSize:12 }}>— только просмотр</span>
                    </div>
                    <PlayersTable
                      players={allPlayers.filter(p=>p.manager_id===viewingManager)}
      ...
      {tab==="platforms"&&(
        <div style={{ padding:"16px 20px" }}>
          <div style={{ border:`1px solid ${T.border}`,borderRadius:10,overflow:"hidden" }}>
            <table style={{ width:"100%",borderCollapse:"collapse" }}>
              <thead><tr>{["Платформа","Гео","Дата","Мин. деп","Цель СЧ","Капа","Мои лиды","Нужно добрать"].map(h=><th key={h} style={S.th}>{h}</th>)}</tr></thead>
              <tbody>
                {platformStats.map(p=>{
                  ...
                  return(
                    <tr key={p.id} className="row-hover">
                      <td style={{ ...S.td,fontWeight:600,color:T.text }}>{p.name}</td>
                      <td style={S.td}>{geo?<span style={{ background:"rgba(99,102,241,.15)",color:"#a5b4fc",padding:"2px 8px",borderRadius:6,fontSize:11,fontWeight:600 }}>{geo.name}</span>:"—"}</td>
...
function AdminPage({ onLogout }) {
  ...
  const [pForm, setPForm] = useState({ name:"", target_avg_check:"", min_deposit:"", cap:"", date_added:"", is_active:true, reset_monthly:false, geo_id:"" });
  ...
  const openPlatformForm = (p=null) => { setEditingPlatform(p); setPForm(p?{name:p.name,target_avg_check:p.target_avg_check,min_deposit:p.min_deposit||"",cap:p.cap||"",date_added:p.date_added||"",is_active:p.is_active!==false,reset_monthly:p.reset_monthly||false,geo_id:p.geo_id||""}:{name:"",target_avg_check:"",min_deposit:"",cap:"",date_added:new Date().toISOString().slice(0,10),is_active:true,reset_monthly:false,geo_id:""}); setShowPlatformForm(true); };
  const savePlatform = async () => { if(!pForm.name||!pForm.target_avg_check){ showToast("Заполни поля","error"); return; } const data={name:pForm.name,target_avg_check:Number(pForm.target_avg_check),min_deposit:Number(pForm.min_deposit)||0,cap:pForm.cap?Number(pForm.cap):null,date_added:pForm.date_added||null,is_active:pForm.is_active,reset_monthly:pForm.reset_monthly,geo_id:pForm.geo_id||null}; if(editingPlatform){ await supabase.from("platforms").update(data).eq("id",editingPlatform.id); showToast("Обновлено!"); }else{ await supabase.from("platforms").insert(data); showToast("Добавлено!"); } setShowPlatformForm(false); load(); };
      ...
      {showPlatformForm&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.8)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
          <div className="slide-in" style={{background:"#1a1d27",border:"1px solid #2d3148",borderRadius:14,padding:24,width:"100%",maxWidth:460,boxShadow:"0 24px 64px rgba(0,0,0,.7)"}}>
            ...
            {[["Название *","name","text"],["Цель СЧ (€) *","target_avg_check","number"],["Мин. депозит (€)","min_deposit","number"],["Капа","cap","number"],["Дата добавления","date_added","date"]].map(([l,k,t])=>(
